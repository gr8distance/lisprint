...
pub fn build_bridge_project(proj: &Project) -> Result<PathBuf, String> {
    ...
    // Find lisprint-core path (relative to build dir)
    let lisprint_root = find_lisprint_root()?;
    let core_path = lisprint_root.join("crates").join("core");
...
/// Find the lisprint installation root (where crates/core lives)
fn find_lisprint_root() -> Result<PathBuf, String> {
    // Check if we're running from a cargo install or development
    // First try: relative to the current executable
    if let Ok(exe) = std::env::current_exe() {
        // Development: target/debug/lisprint or target/release/lisprint
        // → go up to workspace root
        let mut dir = exe.parent().unwrap().to_path_buf();
        for _ in 0..3 {
            if dir.join("crates").join("core").join("Cargo.toml").exists() {
                return Ok(dir);
            }
            if !dir.pop() {
                break;
    ...
    // Fallback: check if lisprint-core is available as a crate
    // This would be the case for cargo install
    Err("Cannot find lisprint-core. Make sure lisprint is installed from source.".to_string())
}
